...
SELECT *
FROM movie
WHERE id OR title OR year OR date_published OR duration OR country OR worlwide_gross_income OR languages OR production_company IS NULL;
...
SELECT COUNT(id) AS no_of_movies, country
FROM movie
...
SELECT COUNT(movie_id) AS movie_produce_count, genre
FROM genre g
GROUP BY genre
ORDER BY movie_produce_count DESC;
...
SELECT COUNT(title), median_rating
FROM movie m
JOIN
	ratings r
    ON m.id=r.movie_id
...
SELECT * FROM names
WHERE name OR height OR date_of_birth OR known_for_movies IS NULL; 
...
SELECT n.name AS actor_name, SUM(r.total_votes) as total_votes, COUNT(r.movie_id) as movie_count, ROUND(SUM(avg_rating*total_votes)/SUM(total_votes),2) AS actor_avg_rating, 
